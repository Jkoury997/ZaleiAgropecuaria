...
export async function GET(req) {
    try {
        ...
        const response = await fetch(`${NEXT_PUBLIC_URL_API_AVICOLA}/api/Catalogo/Galpones`, {
            method: 'GET',
            headers: {
        ...
        if (response.ok) {
            ...
            return NextResponse.json({ error: responseData.Mensaje || 'Failed to retrieve galpones' }, { status: response.status });
        }
    } catch (error) {

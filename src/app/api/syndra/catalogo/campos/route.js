...
export async function GET(req) {
    try {
        ...
        const response = await fetch(`${NEXT_PUBLIC_URL_API_AVICOLA}/api/Catalogo/Campos`, {
            method: 'GET',
            headers: {
        ...
        if (response.ok) {
            ...
            return NextResponse.json({ error: responseData.Mensaje || 'Failed to retrieve campos' }, { status: response.status });
        }
    } catch (error) {
